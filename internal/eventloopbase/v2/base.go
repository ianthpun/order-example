...
import (
	...
	"github.com/dapperlabs/dapper-flow-api/internal/dal/v2"
	"github.com/dapperlabs/dapper-flow-api/internal/errorutils"
	"github.com/dapperlabs/dapper-flow-api/internal/eventloopbase/v2/middleware"
	circleerrs "github.com/dapperlabs/dapper-flow-api/internal/payprovider/circle/errors"
	chainalysiserrs "github.com/dapperlabs/dapper-flow-api/pkg/chainalysis/shared/errors"
	"github.com/dapperlabs/dapper-flow-api/pkg/flowutils"
	"github.com/dapperlabs/dapper-flow-api/pkg/tracing"
)
...
func (b *BaseEventLoop) RegisterWithErrHandler(
	...
	b.eventHandlers[topic] = wrap(eventHandler, b.DisablePersistence)
...
func IsTransient(err error) bool {
	...
	// check if it's a transient Flow error
	if flowutils.IsTransient(err) {
		return true
	}
	...
	// Check for transient Postgres errors
	if errors.Is(err, dalerrs.ErrTransient) {
		return true
	}

	if errorutils.IsNetworkError(err) || errors.Is(err, dalerrs.ErrDALNetworkFailure) {
		// Transient network error.
		return true
	}

	// Check if the error is a transient error from Circle
	if errors.Is(err, circleerrs.ErrTransient) {
		return true
	}

	// Check if the error is a transient error from Chainalysis
	if errors.Is(err, chainalysiserrs.ErrTransient) {
		return true
	}

	// if an eventloop fails because a cb is open, this should be retried
	if errors.Is(err, circuitbreaker.ErrOpen) {
		return true
	}

	// Check if the error is a transient error
	return errorutils.IsTransient(err)
}
...
func wrap(handler middleware.EventHandlerFunc, disablePersistence bool) func(
	context.Context,
	interface{},
	[]byte,
) *eventclient.EventProcessResult {
	return func(ctx context.Context, tx interface{}, bytes []byte) *eventclient.EventProcessResult {
		if !disablePersistence {
			dalTx, ok := tx.(*pg.Tx)
			if !ok {
				return Fail("transaction object provided by eventclient was not of type *pg.Tx")
			}

			ctx = dal.ContextWithTx(ctx, dalTx)
		}
...
func RegisterEventLoopMid(topic string, config eventclient.Config) interface{} {
	if tracing.Enabled() {
		return middleware.NewEventTracingMiddleware(topic, config)
	}
